...
namespace TraoDoiDo.Database
{
    public class MoTaHangHoaDao : ThuocTinhDao
    {
        ...
        {
            string sqlStr = $"DELETE FROM {moTaSanPhamHeader} WHERE {sanPhamID} = '{idSanPham}' ";
            dbConnection.ThucThi(sqlStr);
        }
        ...
        public List<MoTaHangHoa> TimKiemDanhSachAnhVaMoTaBangId(string idSanPham)
        {
            string sqlStr = $"SELECT {moTaSanPhamHeader}.{moTaSanPhamLinkAnh}, {moTaSanPhamHeader}.{moTaSanPhamMoTa} FROM {sanPhamHeader} INNER JOIN {moTaSanPhamHeader}"+
                       $" ON {sanPhamHeader}.{sanPhamID} = {moTaSanPhamHeader}.{sanPhamID} WHERE {sanPhamHeader}.{sanPhamID} = '{idSanPham}' ";
