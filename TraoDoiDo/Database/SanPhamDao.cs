...
namespace TraoDoiDo.Database
{
    public class SanPhamDao : ThuocTinhDao
    {
        ...
        public List<SanPham> LoadSanPham(string idNguoiMua) //Xét yêu thích (lên wpnl_hienThi)
        {
            ...
            string sqlStr = $@" 
                        SELECT {sanPhamHeader}.{sanPhamID}, {sanPhamHeader}.{sanPhamTen}, {sanPhamHeader}.{sanPhamAnh}, {sanPhamHeader}.{sanPhamGiaGoc}, {sanPhamHeader}.{sanPhamGiaBan}, {sanPhamHeader}.{sanPhamNoiBan}, {sanPhamHeader}.{sanPhamSoLuotXem}, {nguoiDungHeader}.{nguoiDungID}, {sanPhamHeader}.{sanPhamLoai}, {danhMucHeader}.{danhMucNguoiMua} 
                        FROM {sanPhamHeader} 
        ...
        public List<SanPham> LoadToanBoSanPham(string idNguoiDang)
        {
            string sqlStr = $@"
                        SELECT {sanPhamID}, {sanPhamTen},{sanPhamAnh},{sanPhamLoai},{sanPhamSoLuong},{sanPhamSLDaBan},{sanPhamGiaGoc},{sanPhamGiaBan},{sanPhamPhiShip},{sanPhamNgayDang} 
                        FROM {sanPhamHeader} 
                        WHERE {sanPhamHeader}.{sanPhamIdNguoiDang} = '{idNguoiDang}' 
                    ";
            dsSanPham = new List<SanPham>();
            bangKetQua = dbConnection.LayNhieuDongDuLieu<string>(sqlStr);
            foreach (var dong in bangKetQua)
        ...
        {
            string sqlStr = $@"SELECT MAX({sanPhamID}) AS IdMax FROM {sanPhamHeader}";
        ...
        {
            string sqlStr = $@"
                        {sanPhamHeader}.{sanPhamIdNguoiDang},{sanPhamHeader}.{sanPhamID},{sanPhamHeader}.{sanPhamSLDaBan},{sanPhamHeader}.{sanPhamSoLuong},{trangThaiHeader}.{trangThaiNgay},{trangThaiHeader}.{trangThaiTongThanhToan}
