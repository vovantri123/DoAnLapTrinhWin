...
namespace TraoDoiDo.Database
{
    public class QuanLyDonHangDao:ThuocTinhDao
    {
        ...
        {
            string sqlStr = $@"
                INSERT INTO {quanLyHeader} ({quanLyIdNguoiDang},{quanLyIdNguoiMua},{quanLyIdSanPham},{quanLyTrangThai},{quanLyLyDo})
        ...
        {
            string sqlStr = $@"
                DELETE FROM {quanLyHeader} 
                WHERE {quanLyIdSanPham} = {ql.IdSanPham} AND {quanLyIdNguoiMua} = {ql.IdNguoiMua}
            ";
            dbConnection.ThucThi(sqlStr);
        }
        ...
        {
            string sqlStr = $@" 
                SELECT {sanPhamHeader}.{sanPhamID},{quanLyHeader}.{quanLyIdNguoiMua}, {sanPhamHeader}.{sanPhamTen}, {sanPhamHeader}.{sanPhamAnh}, {trangThaiHeader}.{trangThaiSoLuongMua}, {sanPhamHeader}.{sanPhamGiaBan}, {sanPhamHeader}.{sanPhamPhiShip}, {trangThaiHeader}.{trangThaiTongThanhToan}, {quanLyHeader}.{quanLyLyDo}
                FROM {quanLyHeader} 
                INNER JOIN {nguoiDungHeader} ON {quanLyHeader}.{quanLyIdNguoiMua} = {nguoiDungHeader}.{nguoiDungID} 
                INNER JOIN {sanPhamHeader} ON {quanLyHeader}.{quanLyIdSanPham} = {sanPhamHeader}.{sanPhamID}                  
            ...
            foreach (var dong in bangKetQua)
                dsDonHang.Add(new QuanLyDonHang("", idNguoiDang, dong[1], dong[0], "", dong[8], dong[2], dong[3], dong[4], dong[5], dong[6], dong[7]));
            return dsDonHang;
        } 
    }
}
